...
//daveti: debug
	if (uf_debug)
		pr_info("usbfilter: scsi-sd - into [%s] for queue [%p] request [%p]\n",
			__func__, q, rq);
	/*
...
//daveti: debug
	if (uf_debug) {
		if (in_interrupt())
			pr_info("usbfilter: scsi-sd - [%s] in IRQ ctx for dev [%s] with a new request [%p]\n",
				__func__, dev_name(&sdkp->dev), rq);
		else
			pr_info("usbfilter: scsi-sd - [%s] in process ctx pid [%i] comm [%s] for dev [%s] with a new request [%p]\n",
				__func__, current->pid, current->comm, dev_name(&sdkp->dev), rq);
	}
...
//daveti: debug
	if (uf_debug)
		pr_info("usbfilter: scsi-sd - [%s] set dev_name [%s]\n",
